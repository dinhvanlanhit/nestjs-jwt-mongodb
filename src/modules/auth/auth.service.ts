...
export class AuthService {
  ...
  constructor(
    ...
    this.cryptr = new Cryptr(process.env.SERVER_ENCRYPT_JWT_SECRET);
  }

  async createAccessToken(userId: string) {
    // const accessToken = this.jwtService.sign({userId});
    const accessToken = sign({userId}, process.env.SERVER_JWT_SECRET , { expiresIn: process.env.SERVER_JWT_EXPIRATION });
  ...
  private jwtExtractor(request) {
    ...
    const cryptr = new Cryptr(process.env.SERVER_ENCRYPT_JWT_SECRET);
    if (token) {
      try {
